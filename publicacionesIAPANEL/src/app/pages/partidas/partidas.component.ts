...
export class PartidasComponent implements OnInit,  AfterViewInit {
 ...
 tratarError(msg : any){
  ...
  crear(): void {
    ...
    this.api_serv.postQuery('partidas', datos)
    .subscribe({
      next(data : any) {
        console.log(data);
        
        that.myForm.reset({});
        that.listado.push(data.partida);
  ...
  editar(): void {
    ...
    this.api_serv.putQuery(`partidas/${ this.selectObj.id }`, datos)
    .subscribe({
      next(data : any) {
        console.log(data);
        
        that.listado[that.selectObjIndex] = data.partida;
  ...
  eliminar( item, index: number ) {
    ...
    Swal.fire({
      ...
      if ( resp.value ) {
        ...
        this.api_serv.deleteQuery(`partidas/${ item.id }`)
        .subscribe({
          next(data : any) {
            console.log(data);
