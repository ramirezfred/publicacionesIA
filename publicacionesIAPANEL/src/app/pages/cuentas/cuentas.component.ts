 ...
 tratarError(msg : any){
  ...
  crearFamilia(forma : NgForm){
    ...
    if(forma.invalid){
      ...
      this.api_serv.postQuery('familias', datos)
      .subscribe({
        next(data : any) {
          console.log(data);
          
          that.myForm.patchValue({familia_id : ''});
          that.getFamilias();
  ...
  aEliminarFamilia(){
    if(this.myForm.value.familia_id == '' || this.myForm.value.familia_id == null){
      Swal.fire({
        title: 'Error',
      ...
      for(var i = 0; i < this.familias.length; ++i){
        if(this.familias[i].id == this.myForm.value.familia_id){
          this.eliminarFamilia(this.familias[i],i);
        }
      }
    }
  }

  eliminarFamilia( item, index: number ) {
    ...
    Swal.fire({
      ...
      if ( resp.value ) {
        ...
        this.api_serv.deleteQuery(`familias/${ item.id }`)
        .subscribe({
          next(data : any) {
            console.log(data);
            
            //that.familias.splice(index, 1);
            that.getFamilias();
            that.myForm.patchValue({familia_id : ''});
  ...
  crear(): void {
    ...
    this.api_serv.postQuery('cuentas', datos)
    .subscribe({
      next(data : any) {
        console.log(data);
        
        that.myForm.reset({});
        that.listado.push(data.cuenta);
  ...
  editar(): void {
    ...
    this.api_serv.putQuery(`cuentas/${ this.selectObj.id }`, datos)
    .subscribe({
      next(data : any) {
        console.log(data);
        
        that.listado[that.selectObjIndex] = data.cuenta;
  ...
  eliminar( item, index: number ) {
    ...
    Swal.fire({
      ...
      if ( resp.value ) {
        ...
        this.api_serv.deleteQuery(`cuentas/${ item.id }`)
        .subscribe({
          next(data : any) {
            console.log(data);
